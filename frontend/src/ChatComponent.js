...
function ChatComponent() {
  ...
  const handleSend = async (e) => {
    ...
    const userMessage = { role: 'user', text: input, isOld: false };
    ...
    try {
      ...
      const krishnaMessage = { role: 'krishna', text: res.data.krishnaGuidance, isOld: false };
      setMessages(prev => [...prev, krishnaMessage]);
    } catch (err) {
      setMessages(prev => [...prev, { role: 'krishna', text: "Kshama karein, Parth. Sampark toot gaya hai.", isOld: false }]);
    } finally {
      setLoading(false);
    }
  };

  // Background style define kar rahe hain
  const divineBgStyle = {
    position: 'relative',
    flex: 1,
    padding: '20px',
    ...
    <div className="app-container">
      <header className="header">
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: '1.2rem' }}>KRISHNA SAMVAD</h1>
      ...
      {/* Yahan style apply kiya hai */}
      <div className="chat-window" style={divineBgStyle}>
        {messages.length === 0 && !loading && (
          <div style={{ textAlign: 'center', marginTop: '50px', color: '#999', position: 'relative', zIndex: 1 }}>
            "Apne man ka bhay tyaago, {userName}. Pucho jo puchna hai."
          </div>
        )}
        
        {messages.map((msg, i) => (
          <div key={i} className={`message ${msg.role === 'user' ? 'user-message' : 'krishna-message'}`} style={{ zIndex: 1 }}>
        ...
        {loading && (
          <div style={{ alignSelf: 'flex-start', color: '#ea580c', fontStyle: 'italic', padding: '10px', zIndex: 1 }}>
            Krishna is thinking...
          </div>
        )}
        <div ref={chatEndRef} />
      ...
      <form onSubmit={handleSend} className="input-area">
        ...
        />
        <button type="submit" disabled={loading}>BHEJEIN</button>
      </form>
    </div>
  );
}
